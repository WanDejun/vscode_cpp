...
COORD coord = { 0,0 };
//双缓冲处理显示
COORD dwBufferSize = { LENGTH };
DWORD bytes = 0;
CHAR_INFO output_data[(LENGTH + 5) << 1][(LENGTH + 5) << 1];
bool buf_flag = 0;
...
void _init_() {
    ...
    CharInfo.Attributes = FOREGROUND_BLUE | BACKGROUND_BLUE;
    CharInfo.Char.AsciiChar = '*';
    for (int i = 0; i < ((LENGTH + 2) << 1); i++) outline[i] = CharInfo; //边界字符串初始化
    
    for (int i = 0; i < LENGTH; i++) {
        output_data[i][0] = CharInfo;
        output_data[i][(LENGTH + 1) << 1 | 1] = CharInfo;
    }

    CharInfo.Char.AsciiChar = ' ';
    for (int i = 0; i < LENGTH; i++) {
        output_data[i][1] = CharInfo;
        output_data[i][(LENGTH + 1) << 1] = CharInfo;
    }
...
void fill_in_data() {
    ...
    for (x = 0; x < LENGTH; x++) {
        for (y = 1; y <= LENGTH; y++) {
            output_data[x][y << 1] = CharInfo;
            output_data[x][y << 1 | 1] = CharInfo;
        }
    }
    for (x = snake_Head.x, y = snake_Head.y; 1; xx = mx_next[x][y].x, yy = mx_next[x][y].y, x = xx, y = yy) {
        if (x == snake_Head.x && y == snake_Head.y) CharInfo.Attributes = FOREGROUND_RED | BACKGROUND_RED; //设置蛇头颜色
        else CharInfo.Attributes = FOREGROUND_GREEN | BACKGROUND_GREEN; //身体颜色
        CharInfo.Char.AsciiChar = 219;
        output_data[x][(y + 1) << 1] = CharInfo;
        output_data[x][(y + 1) << 1 | 1] = CharInfo;
        
        if (x == snake_Tail.x && y == snake_Tail.y) break;
    }
    CharInfo.Attributes = FOREGROUND_BLUE | BACKGROUND_BLUE;
    CharInfo.Char.AsciiChar = '#';
    output_data[candy.x][(candy.y + 1) << 1] = CharInfo;
    output_data[candy.x][(candy.y + 1) << 1 | 1] = CharInfo;
...
void show() {
    ...
    coord.Y = 0;
    WriteConsoleOutput(*out_buf, outline, (LENGTH + 2) << 1, coord, &bytes);
    for (i = 0; i < LENGTH; i++) {
        coord.Y = i + 1;
        WriteConsoleOutputCharacterA(*out_buf, output_data[i], (LENGTH + 2) << 1, coord, &bytes);
    }
    coord.Y++;
    WriteConsoleOutputCharacterA(*out_buf, outline, (LENGTH + 2) << 1, coord, &bytes);
    coord.Y++;
    WriteConsoleOutputCharacterA(*out_buf, str, 10, coord, &bytes);
    //设置新的缓冲区为活动显示缓冲
